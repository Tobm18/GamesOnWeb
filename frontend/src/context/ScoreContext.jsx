...
export function ScoreProvider({ children }) {
  ...
  const scores = React.useMemo(() => {
    const gameIds = ["keep-it-alive", "miage-surfers", "puissance4"];
    const results = {};
    ...
    gameIds.forEach((gameId) => {
      ...
      const personalScore = user
        ? rawScores.find(
            (s) => s.game === gameId && s.playerName === user.username,
          )
        : null;
  ...
  return (
    <ScoreContext.Provider
      value={{ scores, loading, refreshScores: fetchAllScores, updateScore }}
    >
      {children}
    </ScoreContext.Provider>
  );
